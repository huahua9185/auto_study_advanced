...
def test_required_course_parsing():
    """测试必修课页面解析功能"""
    ...
    try:
        ...
        # 3. 测试必修课页面访问
        print(f"\n【步骤3】访问必修课页面...")
        print(f"URL: {Config.REQUIRED_COURSES_URL}")
        ...
        # 5. 测试当前解析逻辑
        print(f"\n【步骤5】测试当前的必修课解析逻辑...")
        courses = parser.parse_required_courses()
        ...
        print(f"- required_course_debug.png (页面截图)")
        print(f"- required_course_debug.html (页面HTML)")
        print(f"- required_course_test_report.json (测试报告)")
        print(f"{'='*80}")
...
def test_alternative_parsing(page):
    ...
    try:
        ...
        for selector in my_study_selectors:
            elements = page.locator(selector)
            if elements.count() > 0:
...
def save_debug_info(page, courses):
    ...
    try:
        # 保存截图
        page.screenshot(path="required_course_debug.png")
        print("✅ 截图已保存: required_course_debug.png")
        
        # 保存HTML
        html_content = page.content()
        with open("required_course_debug.html", "w", encoding="utf-8") as f:
            f.write(html_content)
        print("✅ HTML已保存: required_course_debug.html")
        
        # 保存解析结果
        with open("required_course_parsed_data.json", "w", encoding="utf-8") as f:
            json.dump(courses, f, ensure_ascii=False, indent=2)
        print("✅ 解析数据已保存: required_course_parsed_data.json")
...
def generate_test_report(page_analysis, courses):
    ...
    if not any(nav['visible'] for nav in page_analysis['navigation'] if '我的学习' in nav['text']):
        report['issues_found'].append("未找到'我的学习'导航")
    
    if page_analysis['basic_info']['url'].find('tool_box') != -1:
        report['issues_found'].append("当前页面似乎是选课工具页面，不是学习页面")
    
    # 保存报告
    with open("required_course_test_report.json", "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    
    print("✅ 测试报告已生成: required_course_test_report.json")
